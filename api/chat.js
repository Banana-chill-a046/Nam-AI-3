...
export default async function handler(req, res) {
    ...
    if (!apiKey) {
        return res.status(500).json({ error: "Lỗi hệ thống: Chưa cấu hình API_KEY trên Vercel." });
    }
    ...
    try {
        ...
        if (imageBase64) {
            try {
                ...
                return res.status(400).json({ error: "Định dạng hình ảnh không hợp lệ." });
            }
        }
